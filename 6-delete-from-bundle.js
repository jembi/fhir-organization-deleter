...
import { checkAndDeleteResource } from './fhir/index.js';
import { bulkDeleteResourcesForIds } from './clickhouse/index.js';

const BUNDLE_FILE = process.env.BUNDLE_FILE || './bundles.json';
...
async function processBundleFile() {
  try {
    ...
    const bundles = JSON.parse(bundleContent);
...
async function deleteSingleVersionResources() {
  try {
    ...
    for (const resourceType of RESOURCE_DELETE_ORDER) {
      const fileName = `single-version-${resourceType.toLowerCase()}-resources.csv`;
